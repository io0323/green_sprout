...
class AdvancedAnalysisEngine {
  ...
  Future<void> initialize() async {
    try {
      ...
      _primaryModel = await Interpreter.fromAsset('assets/models/tea_model.tflite');
  ...
  AnalysisResult _analyzeByTexture(img.Image image) {
    ...
    // エッジ検出
    final edges = img.convolution(gray, [
      -1, -1, -1,
      -1,  8, -1,
      -1, -1, -1
    ]);
   ...
   * 複数の結果を統合
   */
  AnalysisResult _combineResults(List<AnalysisResult> results, ImageQuality quality) {
    // 画像品質に基づく重み付け
    ...
    for (int i = 0; i < results.length; i++) {
      ...
      growthStageVotes[results[i].growthStage] = 
          (growthStageVotes[results[i].growthStage] ?? 0) + weight * results[i].confidence;
      healthStatusVotes[results[i].healthStatus] = 
          (healthStatusVotes[results[i].healthStatus] ?? 0) + weight * results[i].confidence;
    }
    ...
    final finalConfidence = (growthStageVotes[bestGrowthStage]! + 
                           healthStatusVotes[bestHealthStatus]!) / 2;
